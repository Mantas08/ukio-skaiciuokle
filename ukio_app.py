        ...
        for page in pdf.pages:
            for table in (page.extract_tables() or []):
                ...
                for j, c in enumerate(hdr):
                    if any(k in c for k in ["preke", "paslauga", "pavadinimas"]):
                        pi = j
                    elif any(k in c for k in ["mato", "vnt", "matav"]):
                        vi = j
                    elif "kiekis" in c or "kiek" in c:
                        ki = j
                    elif "kaina" in c and si is None:
                        kai = j
                    elif "suma" in c:
                        si = j
                if pi is None:
                ...
                for row in table[hdr_idx + 1:]:
                    ...
                    if not preke:
                        continue
                    skip_kw = ["suma be", "pvm", "viso", "moketi", "is viso"]
                    if any(k in preke.lower() for k in skip_kw):
                        continue
