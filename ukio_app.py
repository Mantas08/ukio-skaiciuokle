...
st.markdown(
    ...
    <div class="app-title">
        ...
        <p>Laukai • Išlaidos • Sandėlis • Pajamos • Pelningumas • PDF importas su atskirais parseriais</p>
    </div>
    """,
    unsafe_allow_html=True,
)
...
RUSIS_MAP = {
    "herbicidas": [
        "mustang", "axial", "derby", "corum", "dash", "pantera", "butisan", "herold", "lentipur",
        "sekator", "biathlon", "husar", "stomp", "starane", "focus", "elite", "elit", "belkar",
        "tombo", "nexa", "kestrel", "agrotop"
    ],
    "fungicidas": [
        "prosaro", "osiris", "adexar", "fandango", "skyway", "input", "navura", "caramba", "tilt",
        "amistar", "acanto", "falcon", "opus", "signum"
    ],
    "insekticidas": [
        "karate", "decis", "fastac", "fury", "biscaya", "proteus", "mavrik", "pirimor", "vantex"
    ],
    "reguliatorius": [
        "moddus", "ccc", "terpal", "medax", "cerone", "prodax", "manipulator"
    ],
    "trasos": [
        "npk", "amonio", "kas", "karbamidas", "urea", "salietra", "sulfatas", "superfosfat", "kalio",
        "dap", "map", "kcl", "k40", "kalcio", "magnis", "sulfur", "siera", "mikro", "makro"
    ],
    "seklos": ["sekla", "sėkla", "seed"],
}

SKIP_TEXTS = [
    "suma be pvm", "pvm (21%) suma", "pvm 21", "pvm suma", "viso", "viso su pvm", "iš viso",
    "suma žodžiais", "apmokėti iki", "apmoketi iki", "pardavėjas", "pardavejas", "pirkėjas", "pirkejas",
    "el.pašto adresas", "el. pašto adresas", "bankas", "adresas", "dokumentą išrašė", "dokumenta israse",
    "pagal", "sutartį", "sutarti", "užsakymo nr", "uzsakymo nr", "tiekėjas", "tiekejas"
]
...
def parse_lt_number(v):
    ...
    s = s.strip()
    # paliekam skaičius, minusą, tarpą, kablelį, tašką
    s = re.sub(r"[^0-9,\.\- ]", "", s)
    s = re.sub(r"\s+", " ", s).strip()
    if not s:
        return 0.0

    # LT formatas: 5 550,00 -> 5550.00 ; 15,000 -> 15.000
    s = s.replace(" ", "")
    if "," in s and "." in s:
        # jei būtų mix, laikom kad kablelis decimal, taškai thousand separatoriai
        s = s.replace(".", "")
        s = s.replace(",", ".")
    elif "," in s:
        s = s.replace(",", ".")
...
def isvalyti_produkto_pavadinima(pavadinimas):
    ...
    s = re.sub(r"^\d+[.)]?\s*", "", s)
    # šalinti tik galinį tiekėjo kodą, pvz. (PEHCORU001), (TRGAMSA5001)
    s = re.sub(r"\(([A-Z]{3,}[A-Z0-9]{4,})\)\s*$", "", s).strip()
    # jei liko vien tik trumpa techninė uodega - paliekam originalą
    return s.strip(" -")
...
def koreguoti_eilute_pagal_logika(produktas, vienetas, kiekis, kaina, verte, mode_key):
    ...
    if vienetas not in allowed:
        # universal fallback
        if vienetas not in {"kg", "t", "l", "vnt"}:
            vienetas = "vnt"

    kaina, verte = papildyti_kaina_ir_verte(kiekis, kaina, verte)

    # papildoma korekcija trąšoms, jei kaina akivaizdžiai nesąmonė, o vertė ir kiekis yra geri
...
def parse_agrokoncernas_lines(txt: str, mode_key: str) -> List[dict]:
    ...
    # Ieškom lentelės pradžios po header'io. 
    start = 0
    ...
    row_pattern = re.compile(
        r"^(\d+)\s+(.+?)\s+(kg|t|l|vnt)\s+([0-9][0-9 .]*,[0-9]{3}|[0-9]+,[0-9]{3}|[0-9]+)\s+([0-9][0-9 .]*,[0-9]{2}|[0-9]+,[0-9]{2})\s+([0-9][0-9 .]*,[0-9]{2}|[0-9]+,[0-9]{2})$",
        flags=re.IGNORECASE,
    )
    ...
    for line in candidate_lines:
        ...
        kiekis = parse_lt_number(kiekis_s)
        kaina = parse_lt_number(kaina_s)
        suma = parse_lt_number(suma_s)
        item = koreguoti_eilute_pagal_logika(pavadinimas, vienetas, kiekis, kaina, suma, mode_key)
        ...
        for page in pdf.pages:
            ...
            for table in tables:
                ...
                for i, row in enumerate(table[:8]):
                    row_text = " ".join(txt_norm(c) for c in row if c is not None)
                    if any(x in row_text for x in ["pavadinimas", "prekė", "preke", "aprašymas", "aprasymas", "prekė, paslauga", "preke, paslauga"]):
                ...
                headers = [txt_norm(c) for c in table[header_idx]]
                idx_prod = idx_unit = idx_qty = idx_price = idx_total = None
                for j, h in enumerate(headers):
                    if any(k in h for k in ["pavadinimas", "prekė", "preke", "aprašymas", "aprasymas", "paslauga"]):
                        idx_prod = j
                    elif "mato" in h or h == "vnt" or "mato vnt" in h:
                        idx_unit = j
                    elif "kiekis" in h:
                        idx_qty = j
                    elif "kaina" in h and idx_price is None:
                        idx_price = j
                    elif "suma" in h and idx_total is None:
                        idx_total = j

                if idx_prod is None or idx_qty is None:
                    continue
                ...
                for row in table[header_idx + 1:]:
                    ...
                    if not row_text or yra_suvestines_eilute(row_text):
                        continue
                    produs = row[idx_prod] if idx_prod < len(row) else ""
                    unit = row[idx_unit] if idx_unit is not None and idx_unit < len(row) else "vnt"
                    qty = parse_lt_number(row[idx_qty] if idx_qty < len(row) else 0)
                    price = parse_lt_number(row[idx_price] if idx_price is not None and idx_price < len(row) else 0)
                    total = parse_lt_number(row[idx_total] if idx_total is not None and idx_total < len(row) else 0)
...
def parse_universal_lines(txt: str, mode_key="universal") -> List[dict]:
    ...
    # Universalus regex su vienetu + kiekis + kaina + suma
    pattern = re.compile(
        r"^(?:\d+[.)]?\s+)?(.+?)\s+(kg|t|l|vnt)\s+([0-9][0-9 .]*,[0-9]{3}|[0-9]+,[0-9]{3}|[0-9]+)\s+([0-9][0-9 .]*,[0-9]{2}|[0-9]+,[0-9]{2})\s+([0-9][0-9 .]*,[0-9]{2}|[0-9]+,[0-9]{2})(?:\s+.*)?$",
        flags=re.IGNORECASE,
    )
    ...
    for line in lines:
        ...
        item = koreguoti_eilute_pagal_logika(
            pavadinimas,
            vienetas,
            parse_lt_number(kiekis_s),
            parse_lt_number(kaina_s),
            parse_lt_number(suma_s),
            mode_key,
        )
        if item["Produktas"] and item["Kiekis"] > 0:
            produktai.append(item)
...
def parse_vendor_specific(failas, mode_key: str) -> List[dict]:
    ...
    if mode_key == "agrokoncernas_chemija":
        return parse_agrokoncernas_lines(txt, mode_key)
    if mode_key in {"agrochema", "linas_agro", "scandagra"}:
        # kol kas šitoms įmonėms naudojam universalų išskaidymą,
        # bet architektūra jau paruošta atskiriems parseriams.
        failas.seek(0)
        rez = parse_table_extract_generic(failas, mode_key)
        if rez:
            return rez
        return parse_universal_lines(txt, mode_key)
    if mode_key == "universal":
        failas.seek(0)
        rez = parse_table_extract_generic(failas, mode_key)
        if rez:
            return rez
...
def parinkti_parseri(failas, pasirinktas_mode_key: str):
    txt = gauti_visa_pdf_teksta(failas)
    ...
    if aptiktas_tiekejas == "agrokoncernas":
        # auto heuristika pagal vienetus / raktažodžius
        low = txt_norm(txt)
...
def nuskaityti_pdf(failas, pasirinktas_mode_key: str):
    ...
    # atsarginis bandymas - jei vendor parseris mažai rado
    if not produktai or len(produktai) == 0:
        failas.seek(0)
...
2. SQL Editor paleiskite `setup_atnaujintas_v2.sql`
...
if puslapis == "📊 Suvestinė":
    ...
    if df.empty:
        ...
        with c1:
            rodyti_kpi("Bendra suma", f"{round(bendra_suma, 2)} EUR", "💸")
        with c2:
            rodyti_kpi("Kaina / ha", f"{round(kaina_ha, 2)} EUR", "🌾")
        with c3:
    ...
    if laukai_df.empty:
        ...
        if sdf.empty:
            ...
            with c2:
                prod_ids = st.multiselect(
                    "Produktai",
                    sdf["id"].tolist(),
                    format_func=lambda x: f'{sdf[sdf["id"] == x].iloc[0]["produktas"]} ({sdf[sdf["id"] == x].iloc[0]["kiekis"]} {sdf[sdf["id"] == x].iloc[0]["vienetas"]})',
                    key="nd_produktai",
                )
            with c3:
                    ...
                    if not idarbas:
                        ...
                        for pid in prod_ids:
                            ...
                            sb.table("islaidos").insert({
                                "data": formatuoti_data(idata),
                                "menuo": idata.strftime("%Y-%m"),
                                "lauko_id": int(ilaukas),
                                "darbas": idarbas,
                                "produktas": pr["produktas"],
                                "rusis": pr["rusis"],
                                "norma": float(norma),
                                "vienetas": pr["vienetas"],
                                "kaina_uz_vnt_eur": float(pr["kaina_uz_vnt_eur"]),
                                "sunaudotas_kiekis": round(sun, 4),
                                "suma": round(suma, 2),
                                "pastaba": ipast or "",
                            }).execute()
            ...
            if st.form_submit_button("Pridėti", use_container_width=True):
                if not sp:
                    ...
                    sb.table("sandelis").insert({
                        ...
                        "kiekis": sk,
                        "vienetas": sv,
                        "kaina_uz_vnt_eur": skn,
                        "bendra_verte": round(bv, 2),
    ...
    with t3:
        ...
        st.caption("Dabar yra parserių architektūra su atskirais režimais. Jei reikės daugiau įmonių – galima lengvai pridėti naujas funkcijas ir logikas.")
        if not PDF_OK:
            st.error("Reikia įdiegti pdfplumber biblioteką.")
        else:
            pasirinktas_rezimas = st.selectbox("Importo režimas", list(PRODUCT_TYPE_OPTIONS.keys()))
            st.caption("Jei žinai tiekėją ir dokumento tipą – rinkis konkretų režimą. Jei ne – palik Automatinis.")
            ...
            if pdf_file is not None:
                ...
                if testi:
                    with st.spinner("Nuskaitoma..."):
                        mode_key = PRODUCT_TYPE_OPTIONS[pasirinktas_rezimas]
                        pdf_file.seek(0)
                    ...
                    if not produktai:
                        st.warning("Nepavyko rasti produktų. Šitam tiekėjui tikriausiai reikės dar vieno atskiro parserio – architektūra tam jau paruošta.")
                        with st.expander("Rodyti ištrauktą PDF tekstą diagnostikai"):
                            st.text(parsed_text[:8000])
                    else:
                        ...
                        valid = red_pdf[
                            red_pdf["Produktas"].notna()
                            & (red_pdf["Produktas"].astype(str).str.strip() != "")
                            & (red_pdf["Produktas"].astype(str).str.lower() != "none")
                        ]
                        ...
                        if st.button("Pridėti į sandėlį", type="primary", use_container_width=True):
                            ...
                            for _, row in valid.iterrows():
                                ...
                                if getattr(esamas, "data", None):
                                    ...
                                    # išlaikom esamą savikainą - jei norėsi, vėliau galim dėti svertinį vidurkį
                                    ...
                                    sb.table("sandelis").insert({
                                        ...
                                        "data_prideta": data_str,
                                        "pastaba": f"Iš: {failo_pav}",
                                    }).execute()
                                    prideta += 1
                            try:
                                sb.table("saskaitos").insert({
                                    "failo_pavadinimas": failo_pav,
                                    "saskaitos_numeris": sask_nr or "",
                                    "data_ikelta": datetime.now().strftime("%Y-%m-%d %H:%M"),
                                    "produktu_skaicius": int(len(valid)),
                                    "parserio_rezimas": galutinis_parseris,
                                    "tiekejas": aptiktas_tiekejas,
                                }).execute()
                            except Exception:
                                pass
        ...
        if laukai_df.empty:
            ...
            with st.form("pf", clear_on_submit=True):
                ...
                if st.form_submit_button("Registruoti", use_container_width=True):
                    ...
                    bendras_derlius = float(lr["plotas_ha"]) * float(pder)
                    pajamu_suma = bendras_derlius * float(pkaina)
                    sb.table("pajamos").insert({
                        "data": formatuoti_data(pdata),
                        "lauko_id": int(plaukas),
                        "derlius_t_ha": float(pder),
                        "bendras_derlius_t": round(bendras_derlius, 2),
                        "pardavimo_kaina_eur_t": float(pkaina),
                        "pajamu_suma": round(pajamu_suma, 2),
                        "pastaba": ppast or "",
                    }).execute()
                    st.success(f"Pajamos: {round(pajamu_suma, 2)} EUR")
                    st.rerun()
    with t2:
    ...
    if df.empty:
        ...
        with c2:
            ...
            with pd.ExcelWriter(buf2, engine="openpyxl") as w:
                ...
                suv = pd.DataFrame({
                    "Rodiklis": ["Bendra suma", "Įrašų", "Laukų"],
