        ...
        if sdf.empty: st.warning("Sandelis tuscias. Pridekite produktu.")
            ...
            with st.form("nd", clear_on_submit=True):
                ...
                with c3:
                    ...
                    plotas = li["plotas_ha"]; sun = plotas * inorma; suma = sun * pr["kaina_uz_vnt_eur"]
                    if inorma > 0:
                        st.markdown("**" + str(plotas) + " ha x " + str(inorma) + " = " + str(round(sun, 2)) + " " + pr["vienetas"] + " | Suma: " + str(round(suma, 2)) + " EUR**")
                        if sun > pr["kiekis"]: st.error("Nepakanka sandelyje! Reikia " + str(round(sun, 1)) + ", yra " + str(pr["kiekis"]))
                if st.form_submit_button("Registruoti darba", use_container_width=True):
                    if not idarbas: st.error("Iveskite darba.")
                        ...
                        sb.table("islaidos").insert({"data": idata.strftime("%Y-%m-%d"), "menuo": idata.strftime("%Y-%m"), "lauko_id": int(ilaukas), "darbas": idarbas, "produktas": pr["produktas"], "rusis": pr["rusis"], "norma": inorma, "vienetas": pr["vienetas"], "kaina_uz_vnt_eur": pr["kaina_uz_vnt_eur"], "sunaudotas_kiekis": round(sun, 4), "suma": round(suma, 2), "pastaba": ipast or ""}).execute()
                        nk = max(0, pr["kiekis"] - sun); nv = nk * pr["kaina_uz_vnt_eur"]
