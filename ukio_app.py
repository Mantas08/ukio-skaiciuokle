...
SKIP_ROWS = ["suma be", "pvm", "viso", "moketi", "is viso", "nuolaida", "isankstinio", "viso su", "pvm,", "apmokejimo"]
...
def nuskaityti_pdf(failas):
        ...
        for page in pdf.pages:
            for table in (page.extract_tables() or []):
                ...
                for i, row in enumerate(table):
                    ...
                    if any(k in row_txt for k in HDR_KEYWORDS):
                        hdr_idx = i
                        break

                # Normalizuoti antraste (multi-line)
                ...
                for j, c in enumerate(hdr):
                    cl = c.lower()
                    # Prekes kodas - atskirai, ne produktas
                    if "kodas" in cl or cl.strip() == "nr." or cl.strip() == "nr":
                        kodas_idx = j
                ...
                for row in table[hdr_idx + 1:]:
                    ...
                    if any(k in preke.lower() for k in SKIP_ROWS):
                        continue
                    # Praleisti jei tik skaicius (eilutes nr)
    ...
    produktai = [p for p in produktai if p["Produktas"] and p["Produktas"].lower() not in ["none", "", "nan"] and len(p["Produktas"]) >= 2]
    return produktai
    ...
    if df.empty:
        ...
        with cl:
            ...
            f1 = px.bar(md, x="menuo", y="suma", title="Pagal menesi",
                        labels={"menuo": "Menuo", "suma": "Suma EUR"},
        ...
        with cr:
            if "lauko_pavadinimas" in df.columns:
                ...
                f2 = px.bar(ld, x="lauko_pavadinimas", y="suma", title="Pagal lauka",
                            labels={"lauko_pavadinimas": "Laukas", "suma": "Suma EUR"},
        ...
        with cl2:
            ...
            f3 = px.pie(rd, values="suma", names="rusis", title="Pagal rusi",
        ...
        with cr2:
            ...
            f4 = px.bar(dd, x="darbas", y="suma", title="Pagal darba",
                        labels={"darbas": "Darbas", "suma": "Suma EUR"},
                ...
                if st.button("Issaugoti lauku pakeitimus", width="stretch"):
                    ...
                    for i, row in red.iterrows():
                        ...
                        if changed:
                            ...
                            n += 1
                    if n:
                        st.success("Atnaujinta: " + str(n))
                        st.rerun()
                    else:
                        st.info("Pakeitimu nerasta.")
            with cd:
                tr = red[red["X"] == True]
                if st.button("Istrinti pazymetus (" + str(len(tr)) + ")", width="stretch", disabled=len(tr) == 0, key="dl"):
                    ...
                    st.success("Istrinta: " + str(len(tr)))
                    st.rerun()
            ...
            with st.form("nd", clear_on_submit=True):
                ...
                if st.form_submit_button("Registruoti darba", use_container_width=True):
                    if not idarbas:
                        ...
                        st.success("Darbas registruotas: " + idarbas + " - " + pr["produktas"] + " - " + str(round(suma, 2)) + " EUR. Liko: " + str(round(nk, 1)) + " " + pr["vienetas"])
...
# =============================================================================
    ...
    with t2:
        with st.form("sp", clear_on_submit=True):
            ...
            if st.form_submit_button("Prideti", use_container_width=True):
                if not sp:
                    st.error("Iveskite produkto pavadinima.")
                else:
                    bv = sk * skn
                    ...
                    st.success("Produktas pridetas: " + sp)
                    st.rerun()
        ...
        if not PDF_OK:
            ...
            if pdf_file is not None:
                ...
                if not produktai:
                    ...
                    st.success("Rasta produktu: " + str(len(produktai)) + " | Saskaitos data: " + pdf_data_str)
                    ...
                    red_pdf = st.data_editor(edf, width="stretch", hide_index=True,
                        num_rows="dynamic",
                        column_config={
                            "Produktas": st.column_config.TextColumn("Produktas"),
        ...
        if laukai_df.empty:
            ...
            with st.form("pf", clear_on_submit=True):
                ...
                with c1:
                    ...
                    plaukas = st.selectbox("Laukas", laukai_df["id"].tolist(),
                ...
                if st.form_submit_button("Registruoti", use_container_width=True):
                    ...
                    bd = lr["plotas_ha"] * pder
                    ps = bd * pkaina
                    sb.table("pajamos").insert({
                        "data": pdata.strftime("%Y-%m-%d"), "lauko_id": int(plaukas),
...
# =============================================================================
    ...
    if not idf.empty or not pajdf.empty:
        ...
        fig.update_layout(title="Islaidos ir pajamos", barmode="group",
    ...
    if df.empty:
        ...
        with c2:
            ...
            with pd.ExcelWriter(buf2, engine="openpyxl") as w:
                ...
                suv = pd.DataFrame({
                    "Rodiklis": ["Bendra suma", "Irasu", "Lauku"],
