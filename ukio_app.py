...
st.markdown(
    ...
    .stButton > button, .stDownloadButton > button {border-radius: 12px; font-weight: 600; border: none;}
...
RUSIS_MAP = {
    "herbicidas": ["mustang", "axial", "derby", "corum", "dash", "pantera", "butisan", "herold", "lentipur", "sekator", "biathlon", "husar", "stomp", "starane", "focus", "elite", "elit", "belkar", "tombo", "nexa", "kestrel", "agrotop"],
...
SKIP_TEXTS = [
    "suma be pvm", "viso", "viso su pvm", "iš viso", "pvm 21", "pvm (21%) suma", "pvm suma", "suma žodžiais",
    "apmokėti iki", "apmoketi iki", "užsakymo nr", "uzsakymo nr", "tiekėjas", "tiekejas", "pardavėjas",
    "pardavejas", "pirkėjas", "pirkejas", "dokumentą išrašė", "dokumenta israse", "el.pašto adresas",
    "el. pašto adresas", "bankas", "adresas", "pagal", "sutartį", "sutarti"
]
...
def normalizuoti_vieneta(v):
    vv = txt_norm(v)
    mapping = {"kg": "kg", "t": "t", "l": "l", "lt": "l", "ltr": "l", "l.": "l", "vnt": "vnt", "vnt.": "vnt", "v.": "vnt"}
...
def isvalyti_produkto_pavadinima(pavadinimas):
    ...
    s = re.sub(r"^\d+[.)]?\s*", "", s)
    # šalinam tik aiškius vidinius tiekėjo kodus gale, pvz. (PEHCORU001), (TRGAMSA5001)
    ...
    for line in lines[start_idx:]:
        ...
        if m:
            ...
            current_item = koreguoti_eilute_pagal_logika(
                m.group("pavadinimas"),
                m.group("vienetas"),
                parse_lt_number(m.group("kiekis")),
                parse_lt_number(m.group("kaina")),
                parse_lt_number(m.group("suma")),
                mode_key,
            )
            continue
...
def parse_table_extract_generic(failas, mode_key="universal") -> List[dict]:
    produktai = []
    table_settings = {"vertical_strategy": "lines", "horizontal_strategy": "lines", "intersection_tolerance": 8, "snap_tolerance": 5, "join_tolerance": 5}
    with pdfplumber.open(failas) as pdf:
...
def parse_vendor_specific(failas, mode_key: str) -> List[dict]:
    ...
    if mode_key in {"linas_agro", "scandagra", "universal"}:
        failas.seek(0)
        rez = parse_table_extract_generic(failas, mode_key)
        if rez:
            return rez
...
if puslapis == "📊 Suvestinė":
    ...
    with t3:
        ...
        st.caption("Jei žinai tiekėją ar dokumento tipą – rinkis konkretų importo režimą. Jei ne – palik Automatinis.")
        if not PDF_OK:
            ...
            if pdf_file is not None:
                ...
                if testi:
                    ...
                    if not produktai:
                        st.warning("Nepavyko rasti produktų. Jei reikia, įtrauksim dar vieną atskirą parserį konkrečiam tiekėjui.")
