...
# =====================================================
    ...
    with tab_view:
        if laukai_df.empty:
            ...
            edit_laukai = laukai_df[["id", "pavadinimas", "plotas_ha", "kultura", "pastaba"]].copy()
            edit_laukai.insert(0, "Trinti", False)

            st.caption("Redaguokite laukus tiesiai lentelėje.")

            redaguoti_laukai = st.data_editor(
                edit_laukai,
                use_container_width=True,
                hide_index=True,
                column_config={
                    "Trinti": st.column_config.CheckboxColumn("Trinti", default=False, width="small"),
            ...
            bendras_plotas = redaguoti_laukai["plotas_ha"].sum()
            st.caption(f"Viso laukų: {len(redaguoti_laukai)} · Bendras plotas: {bendras_plotas:.2f} ha")
            ...
            with col_s:
                if st.button("Išsaugoti laukų pakeitimus", use_container_width=True):
                    atnaujinta = 0
                    for i, row in redaguoti_laukai.iterrows():
                        orig = edit_laukai.iloc[i]
                        pasikeitė = False
                        for col in ["pavadinimas", "plotas_ha", "kultura", "pastaba"]:
                        ...
                        if pasikeitė:
                            ...
                            atnaujinta += 1
                    if atnaujinta > 0:
                        st.success(f"Atnaujinta laukų: {atnaujinta}")
                        st.rerun()
                    else:
                        st.info("Pakeitimų nerasta.")

            with col_d:
                trinami = redaguoti_laukai[redaguoti_laukai["Trinti"] == True]
                if st.button(f"Ištrinti pažymėtus ({len(trinami)})", use_container_width=True,
                             disabled=len(trinami) == 0, key="trinti_laukus"):
                    for _, row in trinami.iterrows():
                        sb.table("laukai").delete().eq("id", int(row["id"])).execute()
                    st.success(f"Ištrinta laukų: {len(trinami)}")
                    st.rerun()
...
# =====================================================
    ...
    with tab_view:
        ...
        if sand_df.empty:
            ...
            edit_sand = sand_df[["id", "produktas", "rusis", "kiekis", "vienetas",
                                 "kaina_uz_vnt_eur", "bendra_verte", "pastaba"]].copy()
            edit_sand.insert(0, "Trinti", False)

            st.caption("Redaguokite atsargas tiesiai lentelėje.")

            redaguotas_sand = st.data_editor(
                edit_sand,
                use_container_width=True,
                hide_index=True,
                column_config={
                    ...
                    "rusis": st.column_config.SelectboxColumn("Rūšis",
                    ...
                    "kaina_uz_vnt_eur": st.column_config.NumberColumn("Kaina €/vnt", format="%.2f"),
                    "bendra_verte": st.column_config.NumberColumn("Vertė €", format="%.2f", disabled=True),
                    "pastaba": st.column_config.TextColumn("Pastaba"),
                },
                disabled=["id", "bendra_verte"],
            )

            bendra_verte = redaguotas_sand["bendra_verte"].sum()
            st.caption(f"Produktų: {len(redaguotas_sand)} · Bendra vertė: {bendra_verte:,.2f} €")
            ...
            with col_s:
                if st.button("Išsaugoti sandėlio pakeitimus", use_container_width=True):
                    atnaujinta = 0
                    for i, row in redaguotas_sand.iterrows():
                        orig = edit_sand.iloc[i]
                        pasikeitė = False
                        for col in ["produktas", "rusis", "kiekis", "vienetas", "kaina_uz_vnt_eur", "pastaba"]:
                            if str(row[col]) != str(orig[col]):
                                pasikeitė = True
                                break
                        if pasikeitė:
                            nauja_verte = float(row["kiekis"]) * float(row["kaina_uz_vnt_eur"])
                            sb.table("sandelis").update({
                            ...
                            atnaujinta += 1
                    if atnaujinta > 0:
                        st.success(f"Atnaujinta produktų: {atnaujinta}")
                        st.rerun()
                    else:
                        st.info("Pakeitimų nerasta.")

            with col_d:
                trinami = redaguotas_sand[redaguotas_sand["Trinti"] == True]
                if st.button(f"Ištrinti pažymėtus ({len(trinami)})", use_container_width=True,
                             disabled=len(trinami) == 0, key="trinti_sandelio"):
                    for _, row in trinami.iterrows():
                        sb.table("sandelis").delete().eq("id", int(row["id"])).execute()
                    st.success(f"Ištrinta produktų: {len(trinami)}")
                    st.rerun()
...
# =====================================================
    ...
    if df.empty:
        ...
        # Paruošti lentelę redagavimui
        edit_cols = ["id", "data", "lauko_pavadinimas", "darbas", "produktas",
                     "rusis", "norma", "vienetas", "kaina_uz_vnt_eur", "pastaba"]
        existing = [c for c in edit_cols if c in df.columns]
        edit_df = df[existing].copy()
        edit_df.insert(0, "Trinti", False)

        st.caption("Redaguokite reikšmes tiesiai lentelėje. Pažymėkite varnele eilutes, kurias norite ištrinti.")

        redaguota = st.data_editor(
            edit_df,
            use_container_width=True,
            hide_index=True,
            height=500,
        ...
        with col_save:
            if st.button("Išsaugoti pakeitimus", use_container_width=True):
                atnaujinta = 0
                for i, row in redaguota.iterrows():
                    orig = edit_df.iloc[i]
                    pasikeitė = False
                    for col in ["data", "darbas", "produktas", "rusis", "norma", "vienetas", "kaina_uz_vnt_eur", "pastaba"]:
                        if col in row and col in orig and str(row[col]) != str(orig[col]):
                            pasikeitė = True
                            break

                    if pasikeitė:
                        iraso_id = int(row["id"])
                        orig_row = df[df["id"] == iraso_id].iloc[0]
                        lauko_id = orig_row["lauko_id"]
                        laukas_row = laukai_df[laukai_df["id"] == lauko_id]
                        plotas = laukas_row.iloc[0]["plotas_ha"] if not laukas_row.empty else 0

                        norma = float(row["norma"])
                        kaina = float(row["kaina_uz_vnt_eur"])
                        sunaudota = plotas * norma
                        suma = sunaudota * kaina
                        data_str = str(row["data"])
                        menuo = data_str[:7] if len(data_str) >= 7 else ""
                ...
                if atnaujinta > 0:
                    st.success(f"Atnaujinta įrašų: {atnaujinta}")
                    st.rerun()
                else:
                    st.info("Pakeitimų nerasta.")

        with col_del:
            trinami = redaguota[redaguota["Trinti"] == True]
            if st.button(f"Ištrinti pažymėtus ({len(trinami)})", use_container_width=True,
                         disabled=len(trinami) == 0):
                for _, row in trinami.iterrows():
                    sb.table("islaidos").delete().eq("id", int(row["id"])).execute()
                st.success(f"Ištrinta įrašų: {len(trinami)}")
                st.rerun()
